...
store(
    HydraConf(
        help=HelpConf(
            header=run_pipeline.__doc__,
            app_name=__name__,
        )
    )
)
store(recipe = hydra_zen.builds(run_pipeline, populate_full_signature=True), name=__name__)

recipe = hydra_zen.builds(run_pipeline, populate_full_signature=True, zen_partial=True)
